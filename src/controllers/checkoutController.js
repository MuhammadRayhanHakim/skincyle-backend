...
exports.prosesCheckout = async (req, res) => {
  ...
  try {
    const {
      total_bayar, // Nominal saldo internal yang dipotong sebagian (jika ada)
      item_deskripsi,
      nomor_telepon,
      alamat_rumah,
      jenis_pembelian,
      total_pembayaran_gateway, // Nominal wajib bayar via Midtrans
    } = req.body;
    ...
    if (jenis_pembelian === "saldo" && total_pembayaran_gateway === 0) {
      ...
      const riwayatLokal = await RiwayatSaldo.create(
        {
          id_profil: id_user,
          aktivitas: `Belanja: ${item_deskripsi.substring(0, 200)}`,
          jumlah_saldo: total_bayar,
          tipe_transaksi: "keluar",
          status: "SUKSES", // Saldo internal langsung sukses jika saldo cukup
          tanggal: new Date(),
        },
        { transaction: t },
      );
      ...
      const transaksiLokal = await TransaksiBelanja.create(
        {
          id_riwayat: riwayatLokal.id_riwayat,
          nomor_telepon,
          alamat_rumah,
    ...
    // ─── KONDISI 2: PEMBAYARAN VIA MIDTRANS GATEWAY (TAGIHAN > 0) ───
    // 🟢 REFACTOR UTAMA: Mencegah duplikasi baris riwayat. Hanya membuat SATU log aktivitas murni berstatus PENDING
    const riwayatGateway = await RiwayatSaldo.create(
      {
        id_profil: id_user,
        aktivitas: `Belanja via Midtrans: ${item_deskripsi.substring(0, 200)}`,
        jumlah_saldo: total_pembayaran_gateway,
        tipe_transaksi: "keluar",
        status: "PENDING", // Menjamin default awal bernilai PENDING di image_94ba19.png
        tanggal: new Date(),
      },
      { transaction: t },
    );
    ...
    await TransaksiBelanja.create(
      {
        id_riwayat: riwayatGateway.id_riwayat,
        nomor_telepon,
        alamat_rumah,
        jenis_pembelian,
      },
    ...
    // Jika ada kombinasi potong saldo sebagian, kurangi saldonya tapi JANGAN buat baris log riwayat saldo baru agar tidak duplikat
    if (jenis_pembelian === "saldo" && total_bayar > 0) {
      const userProfil = await Profil.findByPk(id_user, { transaction: t });
      if (userProfil && userProfil.total_saldo >= total_bayar) {
        const saldoTerbaru = userProfil.total_saldo - total_bayar;
    ...
    // Mengunci token orderId dengan menyematkan id_riwayat agar reaktif dipanggil Webhook otomatis
    const orderId = `SKC-BILL-${riwayatGateway.id_riwayat}-${Date.now()}`;
